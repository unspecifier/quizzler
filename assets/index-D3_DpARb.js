...
`,e=e.substring(p.length+1),l=!0),!l){const P=this.rules.other.nextBulletRegex(m),S=this.rules.other.hrRegex(m),B=this.rules.other.fencesBeginRegex(m),x=this.rules.other.headingBeginRegex(m),E=this.rules.other.htmlBeginRegex(m);for(;e;){const q=e.split(`
`,1)[0];let w;if(p=q,this.options.pedantic?(p=p.replace(this.rules.other.listReplaceNesting,"  "),w=p):w=p.replace(this.rules.other.tabCharGlobal,"    "),B.test(p)||x.test(p)||E.test(p)||P.test(p)||S.test(p))break;if(w.search(this.rules.other.nonSpaceChar)>=m||!p.trim())u+=`
`+w.slice(m);else{if(f||h.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||B.test(h)||x.test(h)||S.test(h))break;u+=`
`+p}!f&&!p.trim()&&(f=!0),s+=q+`
...
Please report this to https://github.com/markedjs/marked.`,e){const r="<p>An error occurred:</p><pre>"+U(i.message+"",!0)+"</pre>";return n?Promise.resolve(r):r}if(n)return Promise.reject(i);throw i}}}const se=new Xr;function k(t,e){return se.parse(t,e)}k.options=k.setOptions=function(t){return se.setOptions(t),k.defaults=se.defaults,Wt(k.defaults),k};k.getDefaults=Mn;k.defaults=he;k.use=function(...t){return se.use(...t),k.defaults=se.defaults,Wt(k.defaults),k};k.walkTokens=function(t,e){return se.walkTokens(t,e)};k.parseInline=se.parseInline;k.Parser=L;k.parser=L.parse;k.Renderer=Ke;k.TextRenderer=$n;k.Lexer=H;k.lexer=H.lex;k.Tokenizer=Ve;k.Hooks=Ie;k.parse=k;k.options;k.setOptions;k.use;k.walkTokens;k.parseInline;L.parse;H.lex;var Yr=de('<div class="svelte-1d0jhup"><!></div>'),Jr=de('<fieldset class="fancy"><legend> </legend> <!></fieldset>'),eo=de('<h2 style="font-size: 1.5rem; font-weight: medium; margin: 0">Final Score</h2> <div style="font-size: 2rem; font-weight: bold; text-align: center; margin-bottom: 1rem" class="score svelte-1d0jhup"> </div> <table style="margin: auto"><tbody><tr><th>Correct</th><td> </td></tr><tr><th>Incorrect</th><td> </td></tr></tbody></table>',1),no=de('<article><nav class="svelte-1d0jhup"><button type="button" class="svelte-1d0jhup">Previous</button> <div class="timer svelte-1d0jhup">Timer <time class="svelte-1d0jhup"> </time></div> <button type="button" class="svelte-1d0jhup">Next</button></nav> <!></article>');function to(t,e){en(e,!1);let n=Oe(e,"num_questions",12,5),i=G(),r=G(0),o=G([]),a,c=G(),d=G("00:00:00"),l=G(!1);function s(){g(r)<=g(i).length&&On(r)}function u(x){return k(x,{async:!1}).split(/^<h2>.+<\/h2>\n/gm).filter(Boolean)}Qt(()=>{M(i,[...u(wr),...u(kr)]),$t(g(i)),M(i,g(i).slice(0,n())),n(g(i).length),M(o,new Array(n()).fill(void 0)),a=new Date,M(c,setInterval(()=>{M(d,yr(a))},1e3))});function h(x){lt(o,g(o)[g(r)]=x)}yt(()=>(g(l),g(o),g(c)),()=>{M(l,g(o).every(x=>typeof x=="boolean")),g(l)&&clearInterval(g(c))}),bt(),Rt();var p=no(),f=I(p),m=I(f),y=Q(m,2),v=Q(I(y)),P=I(v),S=Q(y,2),B=Q(f,2);qt(B,()=>g(r)<n(),x=>{var E=Jr(),q=I(E),w=I(q),O=Q(q,2);or(O,1,()=>g(i),ir,(X,on,De)=>{var Te=Yr(),an=I(Te),sn=cn(()=>typeof g(o)[g(r)]=="boolean");vr(an,{get src(){return g(on)},onSubmit:h,get answered(){return g(sn)}}),Y(()=>Gn(Te,"hidden",g(r)!==De)),re(X,Te)}),Y(()=>{E.disabled=typeof g(o)[g(r)]=="boolean",fe(w,`Question: ${g(r)+1} of ${n()??""}`)}),re(x,E)},x=>{var E=eo(),q=Q(Si(E),2);const w=cn(()=>g(o).filter(Boolean).length/n()*100<70);Y(()=>Gn(q,"failed",g(w)));var O=I(q);Y(()=>fe(O,`${(g(o).filter(Boolean).length/n()*100).toFixed(2)??""}%`));var X=Q(q,2),on=I(X),De=I(on),Te=Q(I(De)),an=I(Te);Y(()=>fe(an,`${g(o).filter(Boolean).length??""} out of ${n()??""}`));var sn=Q(De),Ft=Q(I(sn)),Ut=I(Ft);Y(()=>fe(Ut,`${g(o).filter(Gt=>!Gt).length??""} out of ${n()??""}`)),re(x,E)}),Y(()=>{m.disabled=g(r)===0,fe(P,g(d)),S.disabled=g(r)===n()||typeof g(o)[g(r)]!="boolean"}),Ge("click",m,()=>On(r,-1)),Ge("click",S,s),re(t,p),nn()}var io=de('<fieldset class="fancy flex flex-col items-center justify-center gap-2"><label for="numquestions">How many questions?</label> <input type="number" id="numquestions" min="1" class="pl-2 py-1 rounded border border-neutral-300 dark:border-neutral-600"> <button class="mt-4">Start</button></fieldset>'),ro=de('<main><header class="mb-6 grid grid-cols-[1fr,max-content,1fr] gap-4 justify-center"><div></div> <hgroup><h1 class="mb-2 text-5xl font-bold">The Quizzler</h1> <p class="text-center text-xl">Medical Coding Edition</p></hgroup> <div class="justify-self-end"><label class="cursor-pointer text-2xl inline-grid justify-center items-center text-center border p-2 pt-1 rounded-full size-12 hover:bg-neutral-200 dark:hover:bg-neutral-700 leading-none"><input type="checkbox" class="hidden"> <small class="text-xs text-center block sr-only">Theme</small></label></div></header> <!></main>');function oo(t,e){en(e,!1);let n=G(225),i=G(!1),r=G(!1);yt(()=>g(r),()=>{document.documentElement.classList.toggle("dark",g(r))}),bt();var o=ro(),a=I(o),c=Q(I(a),4),d=I(c),l=I(d),s=Q(l),u=Q(a,2);qt(u,()=>!g(i),h=>{var p=io(),f=Q(I(p),2),m=Q(f,2);hr(f,()=>g(n),y=>M(n,y)),Ge("click",m,()=>M(i,!0)),re(h,p)},h=>{to(h,{get num_questions(){return g(n)}})}),Y(()=>fe(s,` ${(g(r)?"🌙":"☀️")??""} `)),ur(l,()=>g(r),h=>M(r,h)),re(t,o),nn()}nr(oo,{target:document.getElementById("app")});
